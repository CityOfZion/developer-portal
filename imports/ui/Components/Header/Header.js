...
class Header extends Component {
    ...
    countdownTimer() {
        if (!this.props.reportSummary) return '';
        let string = '';
        const now = moment();
        ...
        if (Roles.userIsInRole(Meteor.userId(), ['developer'])) {
            ...
            const linkClass = (reportSummary.reports && reportSummary.reports.length === 1) ? '' : 'btn btn-danger mr-2';
            const linkTo = (reportSummary.reports && reportSummary.reports.length === 1) ? `/reports/edit/${reportSummary._id}` : '/reports/add';
            string = <Link className={linkClass} to={linkTo}>Report submission ends
                in: {remaining.days()} days {remaining.hours()} hours {remaining.minutes()} mins</Link>;
        } else {
            ...
            <header className="app-header navbar">
                ...
                <ul className="nav navbar-nav ml-auto">
                    ...
                    <li className="nav-item d-md-down-none">
                        <a className="nav-link" href="#" onClick={this.asideToggle}><i
                            className="icon-bell"></i>{unreadAlertsCount > 0 ?
                            <span className="badge badge-pill badge-danger">{unreadAlertsCount}</span> : ''}</a>
                    </li>
                    <li className="nav-item d-md-down-none">
                    ...
                    <li className="nav-item">
                        <Dropdown isOpen={this.state.dropdownOpen} toggle={this.toggle}>
                            <button onClick={this.toggle} className="nav-link dropdown-toggle" data-toggle="dropdown"
                                ...
                                {this.userIcon(Meteor.user().username)}
                            </button>

                            <DropdownMenu className="dropdown-menu-right">
                                <DropdownItem header className="text-center"><strong>Account</strong></DropdownItem>

                                <DropdownItem onClick={this.asideToggle}><i
                                    className="fa fa-bell-o"></i> Updates {unreadAlertsCount > 0 ?
                                    <span className="badge badge-info">{unreadAlertsCount}</span> : ''}</DropdownItem>
                                <DropdownItem><i className="fa fa-tasks"></i> Tasks {unreadTasksCount > 0 ?
                                    <span className="badge badge-danger">{unreadTasksCount}</span> : ''}</DropdownItem>
                                <DropdownItem><i className="fa fa-comments"></i> Comments {unreadCommentsCount > 0 ?
                                    <span className="badge badge-warning">{unreadCommentsCount}</span> : ''}
                                </DropdownItem>

                                <DropdownItem header className="text-center"><strong>Settings</strong></DropdownItem>

                                <DropdownItem onClick={e => this.props.history.push('/profile')}><i
                                    className="fa fa-user"></i>Profile</DropdownItem>
                                <DropdownItem><i className="fa fa-wrench"></i> Settings</DropdownItem>
                                <DropdownItem divider/>
                                <DropdownItem onClick={e => Meteor.logout(() => this.props.history.push('/login'))}><i
